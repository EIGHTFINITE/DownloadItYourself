...
module.exports = function(current, i, iPad, temp) {
    ...
    if ((current.md5 ? true : current.md5 === temp.md5) && (current.file === temp.file || current.file === temp.file + ".disabled") && fs.existsSync((current["folder-override"] ? "../" + current["folder-override"] : "../" + global.list.config.folder) + "/" + current.file)) { // Nothing to update.
        ...
        if (!current.disabled && current.file.endsWith(".disabled")) {
            ...
            fs.rename((current["folder-override"] ? "../" + current["folder-override"] : "../" + global.list.config.folder) + "/" + current.file, (current["folder-override"] ? "../" + current["folder-override"] : "../" + global.list.config.folder) + "/" + temp.file, function(err) {
                if (err) throw err;
    ...
    if (current.file && fs.existsSync((current["folder-override"] ? "../" + current["folder-override"] : "../" + global.list.config.folder) + "/" + current.file)) {
        console.log("[" + iPad + '] Deleting outdated file: "' + current.file + '".');
        fs.unlinkSync((current["folder-override"] ? "../" + current["folder-override"] : "../" + global.list.config.folder) + "/" + current.file);
    ...
    fs.ensureDirSync(current["folder-override"] ? "../" + current["folder-override"] : "../" + global.list.config.folder);
    request(temp.href).pipe(fs.createWriteStream((current["folder-override"] ? "../" + current["folder-override"] : "../" + global.list.config.folder) + "/" + temp.file)).on("finish", function() {
        ...
        if (current["additional-folder"]) {
            fs.copySync((current["folder-override"] ? "../" + current["folder-override"] : "../" + global.list.config.folder) + "/" + temp.file, "../" + current["additional-folder"] + "/" + temp.file);
