...
function allAuthors(author) {
	...
	else if(Array.isArray(author.name) && Array.isArray(author.url)) {
		if(author.name.length !== author.url.length)
			throw new Error("Array sizes do not match");
		var authorString = '';
		for (var i = 0; i < author.name.length; i++) {
			authorString += ', <a href="' + author.url[i] + '" title="' + author.name[i] + '">' + author.name[i] + '</a>';
		}
...
function multiplePermissionSources(sources, current) {
	if(typeof sources === "string")
		return '<a href="' + sources + '" title="“' + current.name + '” permissions quote"><img src="docs/img/source.svg" height="24" alt="(source)" title="“' + current.name + '” permissions quote"></a>';
	var permissionString = '';
	for (var i = 0; i < sources.length; i++) {
		permissionString += '<a href="' + sources[i] + '" title="“' + current.name + '” permissions quote"><img src="docs/img/source.svg" height="24" alt="(source)" title="“' + current.name + '” permissions quote"></a>';
	}
...
module.exports = function(filetype) {
	...
	for (var i = 0; i < 3; i++) {
		// Heading
		if(i === 2)
			readme += '<h2>Java Libraries</h2>';
		else if(i === 0)
			readme += '<h2>Binaries</h2>';
		else
			readme += '<h2>Mods</h2>';

		// Table start
		if(filetype === "html")
			readme += '<table data-sortable>';
		else
			readme += '<table>';
		...
		readme += '<th>Name</th>';
		readme += '<th>Type</th>';
		readme += abbrText(filetype, '<th><abbr title="Whoever originally licensed the work, or the current owner of the IP. Usually a single person or entity.">Author</abbr></th>');
		readme += '<th>License</th>';
		readme += '<th>Source Code</th>';
		readme += abbrText(filetype, '<th><abbr title="Whether we are allowed to include these files in the releases or whether the user has to download them themselves.">Distribution</abbr></th>');
		readme += '<th>Description</th>';
		readme += '<th>Latest File</th>';
		// Table header end
		readme += '</tr></thead>';
		...
		for (var k = 0; k < global.downloads.length; k++) {
			...
			if((i === 0 && (current.type.includes("executable") || current.type === 'binary' )) || (i === 1 && current.type !== 'library' && !(current.type.includes("executable") || current.type === 'binary' )) || (i === 2 && current.type === 'library')) {
				...
				// Icon
				readme += '<td align="center"><a href="' + (current["description-url-override"] ? current["description-url-override"] : current.url) + '" title="' + current.name + '"><img src="docs/img/icon/' + (current.img ? current.img : 'default.svg') + '" width="62" alt="' + current.name + '" title="' + current.name + '"></a></td>';

				// Name
				readme += '<td><a href="' + (current["description-url-override"] ? current["description-url-override"] : current.url) + '" title="' + current.name + '">' + current.name + '</a>' + (current["description-url-override"] && current.url ? ' <a href="' + current.url + '" title="“' + current.name + '” download"><img src="docs/img/download.svg" height="24" alt="(download)" title="“' + current.name + '” download"></a>' : '') + '</td>';

				// Type
				readme += '<td>' + current.type.replace(/\w*/g, function(txt){return txt.charAt(0).toUpperCase() + txt.substr(1)}) + '</td>';

				// Author
				readme += '<td>' + (current.author ? allAuthors(current.author) : '') + '</td>';

				// License
				readme += '<td>' + (current.license ? '<a href="docs/legal/' + current.license.txt + '.txt" title="“' + current.name + '” ' + licenseDedicationTitle(current.license.txt) + '">' + current.license.txt + '</a>' + (current.license.source ? '<a href="' + current.license.source + '" title="“' + current.name + '” license information"><img src="docs/img/source.svg" height="24" alt="(source)" title="“' + current.name + '” license information"></a>' : '') : '');

				// EULA
				readme += (current.eula ? ', <a href="docs/legal/' + current.eula.txt + '.txt" title="“' + current.name + '” EULA">' + current.eula.txt + '</a>' + (current.eula.source ? '<a href="' + current.eula.source + '" title="“' + current.name + '” EULA information"><img src="docs/img/source.svg" height="24" alt="(source)" title="“' + current.name + '” EULA information"></a>' : '') : '')
				...
				// Source Code
				readme += '<td>' + (current.code ? (current.code.url ? '<a href="' + current.code.url + '" title="“' + current.name + '” source code">' + current.code.title + '</a>' : current.code.title) : '') + '</td>';

				// Distribution
				if(filetype === "html")
					readme += '<td align="center">' + (current.distribution === "allowed" ? '<img src="docs/img/check.svg" height="24" alt="OK" title="Distribution Allowed">' : '<img src="docs/img/cross.svg" height="24" alt="No" title="No Distribution Allowed">') + '</td>';
				else
					readme += '<td align="center">' + (current.distribution === "allowed" ? '<a href="##" title="Distribution Allowed"><img src="docs/img/check.svg" height="24" alt="OK" title="Distribution Allowed"></a>' : '<a href="##" title="No Distribution Allowed"><img src="docs/img/cross.svg" height="24" alt="No" title="No Distribution Allowed"></a>') + '</td>';

				// Description
				readme += '<td align="center">' + (current.description ? colorText(filetype, current.description) : '') + '</td>';

				// File
				readme += '<td align="center">' + latestFile(current) + '</td>';
	...
	if(files.length > 0) {
		...
		files.forEach(function(file) {
			...
			if(packageData.repository.url === 'https://github.com/mcavage/node-assert-plus')
				packageData.repository.url = 'https://github.com/joyent/node-assert-plus';
			else if(packageData.repository.url === 'https://github.com/cheeriojs/dom-renderer')
			...
			packageData.name = packageData.repository.url.substring(packageData.repository.url.lastIndexOf('/') + 1);
			packageData.author.url = packageData.repository.url.slice(0,-(packageData.name.length+1));
			if(packageData.description.substring(packageData.description.length-1) !== '.')
				packageData.description += '.';
			packageData.description = packageData.description[0].toUpperCase() + packageData.description.slice(1);
			if(packageData._requiredBy[0] !== '/')
				packageData.description += '<br>Required by ' + (function(){
					...
					for (var i = 0; i < packageData._requiredBy.length; i++) {
						requirements += packageData._requiredBy[i].slice(1) + ', ';
					}
			...
			// Name
			readme += '<td><a href="' + packageData.repository.url + '" title="' + packageData.name + '">' + packageData.name + '</a></td>';

			// Type
			readme += '<td>Module</td>';

			// Author
			readme += '<td><a href="' + packageData.author.url + '" title="' + packageData.author.name + '">' + packageData.author.name + '</a></td>';

			// License
			readme += '<td>' + (packageData.license ? packageData.license : '') + '</td>';

			// Source Code
			readme += '<td>' + packageData.repository.url + '</td>';

			// Distribution
			if(filetype === "html")
				readme += '<td align="center"><img src="docs/img/check.svg" height="24" alt="OK" title="Distribution Allowed"></td>';
			else
				readme += '<td align="center"><a href="##" title="Distribution Allowed"><img src="docs/img/check.svg" height="24" alt="OK" title="Distribution Allowed"></a></td>';

			// Description
			readme += '<td>' + packageData.description + '</td>';

			// File
			readme += '<td align="center"><code>' + packageData._id + '</code></td>';
