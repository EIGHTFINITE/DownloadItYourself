...
module.exports = function(filetype) {
	...
	for (var i = 0; i < 3; i++) {
		...
		for (var k = 0; k < global.downloads.length; k++) {
			...
			if((i === 0 && (current.type.includes("executable") || current.type === 'binary' )) || (i === 1 && current.type !== 'library' && !(current.type.includes("executable") || current.type === 'binary' )) || (i === 2 && current.type === 'library')) {
				...
				// Name
				readme += '\r\n<td><a href="' + (current["description-url-override"] ? current["description-url-override"] : current.url) + '" title="' + current.name + '">' + current.name + '</a>' + (current["description-url-override"] && current.url ? ' <a href="' + current.url + '" title="“' + current.name + '” download"><img src="docs/img/download.svg" height="24" alt="(download)" title="“' + current.name + '” download"></a>' : '') + '</td>';
				...
				// EULA
				readme += '\r\n' + (current.eula ? ', <a href="docs/legal/' + current.eula.txt + '.txt" title="“' + current.name + '” EULA">' + current.eula.txt + '</a>' + (current.eula.source ? '<a href="' + current.eula.source + '" title="“' + current.name + '” EULA information"><img src="docs/img/source.svg" height="24" alt="(source)" title="“' + current.name + '” EULA information"></a>' : '') : '')
				...
				// Distribution
				if(filetype === "html")
					readme += '\r\n<td align="center">' + (current.distribution === "allowed" ? '<img src="docs/img/check.svg" height="24" alt="OK" title="Distribution Allowed">' : '<img src="docs/img/cross.svg" height="24" alt="No" title="No Distribution Allowed">') + '</td>';
				else
					readme += '\r\n<td align="center">' + (current.distribution === "allowed" ? '<a href="##" title="Distribution Allowed"><img src="docs/img/check.svg" height="24" alt="OK" title="Distribution Allowed"></a>' : '<a href="##" title="No Distribution Allowed"><img src="docs/img/cross.svg" height="24" alt="No" title="No Distribution Allowed"></a>') + '</td>';
	...
	if(files.length > 0) {
		...
		files.forEach(function(file) {
			...
			// Icon
			readme += '\r\n<td align="center"><a href="' + packageData.homepage + '" title="' + packageData.name + '"><img src="docs/img/icon/' + (packageData.repository.url.startsWith('https://github.com/request') ? 'request.png' : (packageData.repository.url.startsWith('https://github.com/electron') ? 'electron.svg' : (fs.existsSync('../docs/img/icon/' + packageData.name + '.svg') ? packageData.name + '.svg' : (fs.existsSync('../docs/img/icon/' + packageData.name + '.png') ? packageData.name + '.png' : 'npm.svg')))) + '" width="62" alt="' + packageData.name + '" title="' + packageData.name + '"></a></td>';
			...
			// Distribution
			if(filetype === "html")
				readme += '\r\n<td align="center"><img src="docs/img/check.svg" height="24" alt="OK" title="Distribution Allowed"></td>';
			else
