...
function multiplePermissionSources(sources, current) {
	if(is.string(sources))
		return '<a href="' + sources + '" title="“' + current.name + '” permissions quote"><img src="docs/img/vector/source.svg" height="24" alt="(source)" title="“' + current.name + '” permissions quote"></a>';
	var permissionString = '';
	for (var i = 0; i < sources.length; i++) {
		permissionString += '<a href="' + sources[i] + '" title="“' + current.name + '” permissions quote"><img src="docs/img/vector/source.svg" height="24" alt="(source)" title="“' + current.name + '” permissions quote"></a>';
	}
...
function multipleLicenses(license, name, source) {
	function spdxToHTML(expression) {
		...
		if(is.string(expression.license)) {
			...
			return '<a href="docs/legal/' + license + '.txt" title="“' + name + '” ' + licenseDedicationTitle(license) + '">' + license + '</a>' + (source ? '<a href="' + source + '" title="“' + name + '” license information"><img src="docs/img/vector/source.svg" height="24" alt="(source)" title="“' + name + '” license information"></a>' : '');
		}
		else {
			return spdxToHTML(expression.left) + ' ' + expression.conjunction.toUpperCase() + ' ' + spdxToHTML(expression.right);
		}
	}
	...
	if(is.nonEmptyString(license))
		parsedLicenses = parse(license.replace(/(?<!AND|OR) (?!AND|OR)/g, '--'), {relaxed: true});
	else
...
module.exports = function(filetype) {
	...
	for (var i = 0; i < 3; i++) {
		...
		for (var k = 0; k < global.downloads.length; k++) {
			...
			if((i === 0 && (current.type.includes("executable") || current.type === 'binary' )) || (i === 1 && current.type !== 'library' && !(current.type.includes("executable") || current.type === 'binary' )) || (i === 2 && current.type === 'library')) {
				...
				// EULA
				readme += (current.eula ? ', <a href="docs/legal/' + current.eula.txt + '.txt" title="“' + current.name + '” EULA">' + current.eula.txt + '</a>' + (current.eula.source ? '<a href="' + current.eula.source + '" title="“' + current.name + '” EULA information"><img src="docs/img/vector/source.svg" height="24" alt="(source)" title="“' + current.name + '” EULA information"></a>' : '') : '')
	...
	if(files.length > 0) {
		...
		files.forEach(function(file) {
			...
			else if(packageData.author.url === 'https://github.com/davepacheco')
			...
			else if(!packageData.license && packageData.repository.url === 'https://github.com/kriszyp/json-schema')
				packageData.license = 'AFL-2.1 OR BSD-3-Clause';
			
			// Version corrections
			if(!is.null(packageData._requested.saveSpec)) {
				if(packageData._from.startsWith('github:'))
					packageData._id = packageData._from.slice(7);
				else
					packageData._id = packageData._from;
			}
