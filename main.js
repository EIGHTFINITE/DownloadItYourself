...
var createReadme = require("./func/createReadme.js");
...
fs.readFile("../downloadlist.json", "utf8", function(err, data) {
	...
	// Try to clean up the _temp/_download directory
	fs.rmdir(__dirname + "/_temp/_download", function(err) {
		if(err) {
			if(err.code === "ENOENT") {
				fs.mkdirSync(__dirname + "/_temp/_download");
			}
			else if(err.code === "ENOTEMPTY") {
				console.log("Downloads folder was not properly cleaned last run. Cleaning now.");
				fs.rmdirSync(__dirname + "/_temp/_download", { recursive: true });
				fs.mkdirSync(__dirname + "/_temp/_download");
			}
			else throw err;
		}
...
process.on('exit', function() { // Asynchronous functions do not work beyond this point
	...
	// Clean the _temp/_download directory
	try {
		try {
			fs.rmdirSync(__dirname + "/_temp/_download");
			return;
		}
		catch (err) {
		...
		console.log("Unable to clean downloads folder. Filesystem unresponsive.");
	}
