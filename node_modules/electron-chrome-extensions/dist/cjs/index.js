...
// src/browser/popup.ts
var debug = require("debug")("electron-chrome-extensions:popup");
var supportsPreferredSize = () => {
  const major = parseInt(process.versions.electron.split(".").shift() || "", 10);
  return major >= 12;
};
var _PopupView = class _PopupView {
  constructor(opts) {
...
// src/browser/api/browser-action.ts
var d = require("debug")("electron-chrome-extensions:browserAction");
...
var BrowserActionAPI = class {
  ...
  setupSession(session2) {
    session2.on("extension-loaded", (event, extension) => {
      this.processExtension(extension);
    });
    session2.on("extension-unloaded", (event, extension) => {
      this.removeActions(extension.id);
    });
    this.handleCRXProtocol(session2);
  }
  handleCRXProtocol(session2) {
    if (!session2.protocol.isProtocolHandled("crx")) {
      session2.protocol.handle("crx", this.handleCrxRequest);
    }
  }
...
var debug2 = require("debug")("electron-chrome-extensions:windows");
...
var _WindowsAPI = class _WindowsAPI {
  ...
  observeWindow(window) {
    ...
    debug2(`Observing window[${windowId}]`);
  }
...
var debug3 = require("debug")("electron-chrome-extensions:tabs");
...
var _TabsAPI = class _TabsAPI {
  ...
  observeTab(tab) {
    ...
    debug3(`Observing tab[${tabId}][${tab.getType()}] ${tab.getURL()}`);
  }
...
var debug4 = require("debug")("electron-chrome-extensions:webNavigation");
...
var WebNavigationAPI = class {
  constructor(ctx) {
    ...
    this.sendNavigationEvent = (eventName, details) => {
      debug4(`${eventName} [url: ${details.url}]`);
...
var debug5 = require("debug")("electron-chrome-extensions:store");
var ExtensionStore = class extends import_node_events.EventEmitter {
  constructor(impl) {
    super();
    this.impl = impl;
...
function readRegistryKey(hive, path5, key) {
  ...
  return new Promise((resolve2, reject) => {
    const child = (0, import_child_process.spawn)("reg", ["query", `${hive}\\${path5}`, ...key ? ["/v", key] : []]);
    let output = "";
    let error = "";
...
// src/browser/api/lib/native-messaging-host.ts
var d2 = require("debug")("electron-chrome-extensions:nativeMessaging");
  ...
  for (const filePath of searchPaths) {
    try {
      ...
      if (isValidConfig(config)) {
        d2("read config in %s", filePath, config);
        return config;
      } else {
        d2("%s contained invalid config", filePath, config);
      }
    } catch (error) {
      if (error?.code === "ENOENT") {
        d2("unable to read %s", filePath);
      } else {
...
var NativeMessagingHost = class {
  ...
  async launch(application, extensionId) {
    ...
    if (!config) {
      d2("launch: unable to find %s for %s", application, extensionId);
      this.destroy();
      return;
    }
    const extensionUrl = `chrome-extension://${extensionId}/`;
    if (!config.allowed_origins?.includes(extensionUrl)) {
      d2("launch: %s not in allowed origins", extensionId);
      this.destroy();
      return;
    }
    let isFile = false;
    try {
      ...
      d2("launch: unable to find %s", config.path, error);
    }
    if (!isFile) {
      this.destroy();
      return;
    }
    d2("launch: spawning %s for %s", config.path, extensionId);
    this.process = (0, import_node_child_process.spawn)(config.path, [extensionUrl], {
      shell: false
    });
    this.process.stdout.on("data", this.receive);
    this.process.stderr.on("data", (data) => {
      d2("stderr: %s", data.toString());
    });
    ...
    if (this.pending && this.pending.length > 0) {
      d2("sending %d pending messages", this.pending.length);
...
// src/browser/api/runtime.ts
var RuntimeAPI = class extends import_node_events2.EventEmitter {
  constructor(ctx) {
    super();
    this.ctx = ctx;
    this.hostMap = {};
...
var debug6 = createDebug("electron-chrome-extensions:router");
...
var RoutingDelegate = class _RoutingDelegate {
  constructor() {
    ...
    this.onRouterMessage = async (event, extensionId, handlerName, ...args) => {
      debug6(`received '${handlerName}'`, args);
    ...
    this.onRemoteMessage = async (event, sessionPartition, handlerName, ...args) => {
      debug6(`received remote '${handlerName}' for '${sessionPartition}'`, args);
      const ses = sessionPartition === DEFAULT_SESSION ? getSessionFromEvent(event) : import_electron8.session.fromPartition(sessionPartition);
    ...
    import_electron8.ipcMain.handle("crx-msg", this.onRouterMessage);
    import_electron8.ipcMain.handle("crx-msg-remote", this.onRemoteMessage);
    import_electron8.ipcMain.on("crx-add-listener", this.onAddListener);
    import_electron8.ipcMain.on("crx-remove-listener", this.onRemoveListener);
  }
  static get() {
  ...
  addObserver(observer) {
    ...
    const maybeListenForWorkerEvents = ({
      ...
      if (serviceWorker?.scope?.startsWith("chrome-extension://") && !this.workers.has(serviceWorker)) {
        debug6(`listening to service worker [versionId:${versionId}, scope:${serviceWorker.scope}]`);
...
var ExtensionRouter = class {
  constructor(session2, delegate = RoutingDelegate.get()) {
    ...
    import_electron8.app.on("web-contents-created", (event, webContents2) => {
      if (webContents2.session === this.session && webContents2.getType() === "backgroundPage") {
        debug6(`storing reference to background host [url:'${webContents2.getURL()}']`);
    ...
    session2.serviceWorkers.on(
      ...
      ({ runningStatus, versionId }) => {
        ...
        if (this.extensionHosts.has(serviceWorker)) {
          debug6("%s running status changed to %s", scope, runningStatus);
        } else {
          debug6(`storing reference to background service worker [url:'${scope}']`);
  ...
  filterListeners(predicate) {
    for (const [eventName, listeners] of this.listeners) {
      ...
      if (delta > 0) {
        debug6(`removed ${delta} listener(s) for '${eventName}'`);
      }
    }
  }
  observeListenerHost(host) {
    const hostId = getHostId(host);
    debug6(`observing listener [id:${hostId}, url:'${getHostUrl(host)}']`);
    host.once("destroyed", () => {
      debug6(`extension host destroyed [id:${hostId}]`);
  ...
  addListener(listener, extensionId, eventName) {
    ...
    const extension = session2.getExtension(extensionId);
    if (!extension) {
    ...
    if (existingEventListener) {
      debug6(`ignoring existing '${eventName}' event listener for ${extensionId}`);
    } else {
      debug6(`adding '${eventName}' event listener for ${extensionId}`);
  ...
  removeListener(listener, extensionId, eventName) {
    ...
    if (index >= 0) {
      debug6(`removing '${eventName}' event listener for ${extensionId}`);
  ...
  async onExtensionMessage(event, extensionId, handlerName, ...args) {
    ...
    const extension = extensionId ? eventSession.getExtension(extensionId) : void 0;
    ...
    debug6(`${handlerName} result: %r`, result);
    return result;
  }
  ...
  sendEvent(targetExtensionId, eventName, ...args) {
    ...
    for (const listener of eventListeners) {
      ...
      if (type === "service-worker") {
        ...
        this.session.serviceWorkers.startWorkerForScope(scope).then((serviceWorker) => {
          ...
          debug6("failed to send %s to %s", eventName, extensionId);
          console.error(error);
        });
      } else {
    ...
    debug6(`sent '${eventName}' event to ${sentCount} listeners`);
  }
...
function readPackageJson() {
  const appPath = import_electron9.app.getAppPath();
...
var ElectronChromeExtensions = class extends import_node_events3.EventEmitter {
  ...
  constructor(opts) {
    super();
    const { license, session: session2 = import_electron10.session.defaultSession, modulePath, ...impl } = opts || {};
    checkVersion();
    ...
    this.modulePath = modulePath || import_node_path.default.join(__dirname, "../..");
    this.api = {
    ...
    this.prependPreload();
    if (this.ctx.session !== import_electron10.session.defaultSession) {
      this.handleCRXProtocol(import_electron10.session.defaultSession);
    }
  }
  listenForExtensions() {
  ...
  async prependPreload() {
    ...
    const preloadPath = import_node_path.default.join(this.modulePath, "dist/preload.js");
    ...
    try {
      const stat = await import_node_fs3.promises.stat(preloadPath);
   ...
   */
  handleCRXProtocol(session2) {
    this.api.browserAction.handleCRXProtocol(session2);
  }
  /**
