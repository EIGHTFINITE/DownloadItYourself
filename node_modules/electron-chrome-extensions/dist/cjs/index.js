...
var _PopupView = class _PopupView {
  constructor(opts) {
    ...
    this.maybeClose = () => {
      ...
      if (!import_electron.BrowserWindow.getFocusedWindow()) {
    ...
    this.browserWindow = new import_electron.BrowserWindow({
      show: false,
      frame: false,
      parent: opts.parent,
      movable: false,
...
var resolveExtensionPath = (extension, uri) => {
...
var getIconImage = (extension) => {
  ...
  return iconAbsolutePath ? import_electron2.nativeImage.createFromPath(iconAbsolutePath) : void 0;
};
...
// src/browser/api/browser-action.ts
var debug2 = require("debug")("electron-chrome-extensions:browserAction");
...
var BrowserActionAPI = class {
  constructor(ctx) {
    ...
    this.openPopup = (event, options) => {
      ...
      if (!window || window.isDestroyed()) {
        debug2("openPopup: window %d destroyed", window?.id);
        return;
      }
  ...
  setupSession(session2) {
    ...
    session2.protocol.registerBufferProtocol("crx", this.handleCrxRequest);
  }
  ...
  activateClick(details) {
    ...
    if (this.popup) {
      ...
      this.popup = void 0;
      if (toggleExtension) {
        debug2("skipping activate to close popup");
        return;
      }
    }
    ...
    if (popupUrl) {
      ...
      debug2(`opened popup: ${popupUrl}`);
      this.ctx.emit("browser-action-popup-created", this.popup);
    } else {
      debug2(`dispatching onClicked for ${extensionId}`);
  ...
  activateContextMenu(details) {
    ...
    if (process.env.NODE_ENV === "development" && process.env.DEBUG) {
      append({
        ...
        click: () => {
          debug2(`removing extension "${extension.name}" (${extension.id})`);
  ...
  onUpdate() {
    ...
    queueMicrotask(() => {
      this.queuedUpdate = false;
      debug2(`dispatching update to ${this.observers.size} observer(s)`);
...
var debug3 = require("debug")("electron-chrome-extensions:windows");
...
var _WindowsAPI = class _WindowsAPI {
  ...
  observeWindow(window) {
    ...
    debug3(`Observing window[${windowId}]`);
  }
...
// src/browser/api/tabs.ts
var debug4 = require("debug")("electron-chrome-extensions:tabs");
...
var _TabsAPI = class _TabsAPI {
  ...
  observeTab(tab) {
    ...
    debug4(`Observing tab[${tabId}][${tab.getType()}] ${tab.getURL()}`);
  }
  ...
  onRemoved(tabId) {
    ...
    const win = typeof windowId !== "undefined" && windowId > -1 ? import_electron4.BrowserWindow.getAllWindows().find((win2) => win2.id === windowId) : null;
...
var debug5 = require("debug")("electron-chrome-extensions:webNavigation");
...
var WebNavigationAPI = class {
  constructor(ctx) {
    ...
    this.sendNavigationEvent = (eventName, details) => {
      debug5(`${eventName} [url: ${details.url}]`);
...
var debug6 = require("debug")("electron-chrome-extensions:store");
  ...
  constructor(ctx) {
    ...
    this.buildMenuItemsFromTemplate = (menuItemTemplates) => {
      ...
      const buildFromTemplate = (opts) => {
        ...
        return new import_electron6.MenuItem(opts);
      };
      for (const item of menuItemTemplates) {
...
// src/browser/api/lib/native-messaging-host.ts
var d = require("debug")("electron-chrome-extensions:nativeMessaging");
...
var debug7 = createDebug("electron-chrome-extensions:router");
...
var RoutingDelegate = class _RoutingDelegate {
  constructor() {
    ...
    this.onRouterMessage = async (event, extensionId, handlerName, ...args) => {
      debug7(`received '${handlerName}'`, args);
    ...
    this.onRemoteMessage = async (event, sessionPartition, handlerName, ...args) => {
      debug7(`received remote '${handlerName}' for '${sessionPartition}'`, args);
      const ses = sessionPartition === DEFAULT_SESSION ? getSessionFromEvent(event) : import_electron9.session.fromPartition(sessionPartition);
  ...
  addObserver(observer) {
    ...
    const maybeListenForWorkerEvents = ({
      ...
      if (serviceWorker?.scope?.startsWith("chrome-extension://") && !this.workers.has(serviceWorker)) {
        debug7(`listening to service worker [versionId:${versionId}, scope:${serviceWorker.scope}]`);
...
var ExtensionRouter = class {
  constructor(session2, delegate = RoutingDelegate.get()) {
    ...
    import_electron9.app.on("web-contents-created", (event, webContents2) => {
      if (webContents2.session === this.session && webContents2.getType() === "backgroundPage") {
        debug7(`storing reference to background host [url:'${webContents2.getURL()}']`);
    ...
    session2.serviceWorkers.on(
      ...
      ({ runningStatus, versionId }) => {
        ...
        if (this.extensionHosts.has(serviceWorker)) {
          debug7("%s running status changed to %s", scope, runningStatus);
        } else {
          debug7(`storing reference to background service worker [url:'${scope}']`);
  ...
  filterListeners(predicate) {
    for (const [eventName, listeners] of this.listeners) {
      ...
      if (delta > 0) {
        debug7(`removed ${delta} listener(s) for '${eventName}'`);
      }
    }
  }
  observeListenerHost(host) {
    const hostId = getHostId(host);
    debug7(`observing listener [id:${hostId}, url:'${getHostUrl(host)}']`);
    host.once("destroyed", () => {
      debug7(`extension host destroyed [id:${hostId}]`);
  ...
  addListener(listener, extensionId, eventName) {
    ...
    if (existingEventListener) {
      debug7(`ignoring existing '${eventName}' event listener for ${extensionId}`);
    } else {
      debug7(`adding '${eventName}' event listener for ${extensionId}`);
  ...
  removeListener(listener, extensionId, eventName) {
    ...
    if (index >= 0) {
      debug7(`removing '${eventName}' event listener for ${extensionId}`);
  ...
  async onExtensionMessage(event, extensionId, handlerName, ...args) {
    ...
    debug7(`${handlerName} result: %r`, result);
    return result;
  }
  ...
  sendEvent(targetExtensionId, eventName, ...args) {
    ...
    if (!eventListeners || eventListeners.length === 0) {
      debug7(`ignoring '${eventName}' event with no listeners`);
      return;
    }
    let sentCount = 0;
    for (const listener of eventListeners) {
      ...
      if (type === "service-worker") {
        ...
        this.session.serviceWorkers.startWorkerForScope(scope).then((serviceWorker) => {
          ...
          debug7("failed to send %s to %s", eventName, extensionId);
          console.error(error);
        });
      } else {
    ...
    debug7(`sent '${eventName}' event to ${sentCount} listeners`);
  }
...
var ElectronChromeExtensions = class extends import_node_events3.EventEmitter {
  ...
  constructor(opts) {
    super();
    const { license, session: session2 = import_electron11.session.defaultSession, modulePath, ...impl } = opts || {};
