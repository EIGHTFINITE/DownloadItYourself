...
// src/browser/popup.ts
var debug = __require("debug")("electron-chrome-extensions:popup");
...
// src/browser/api/browser-action.ts
var d = __require("debug")("electron-chrome-extensions:browserAction");
if (!app.isReady()) {
...
var debug2 = __require("debug")("electron-chrome-extensions:windows");
...
var debug3 = __require("debug")("electron-chrome-extensions:tabs");
...
var debug4 = __require("debug")("electron-chrome-extensions:webNavigation");
...
var debug5 = __require("debug")("electron-chrome-extensions:store");
var ExtensionStore = class extends EventEmitter {
  constructor(impl) {
    super();
    this.impl = impl;
...
function readRegistryKey(hive, path5, key) {
  ...
  return new Promise((resolve2, reject) => {
    const child = spawn("reg", ["query", `${hive}\\${path5}`, ...key ? ["/v", key] : []]);
    let output = "";
    let error = "";
...
// src/browser/api/lib/native-messaging-host.ts
var d2 = __require("debug")("electron-chrome-extensions:nativeMessaging");
...
// src/browser/api/runtime.ts
var RuntimeAPI = class extends EventEmitter2 {
  constructor(ctx) {
    super();
    this.ctx = ctx;
    this.hostMap = {};
...
import { app as app4, ipcMain, session } from "electron";
var createDebug = __require("debug");
...
var RoutingDelegate = class _RoutingDelegate {
  constructor() {
    ...
    this.onRemoteMessage = async (event, sessionPartition, handlerName, ...args) => {
      ...
      const ses = sessionPartition === DEFAULT_SESSION ? getSessionFromEvent(event) : session.fromPartition(sessionPartition);
...
var ElectronChromeExtensions = class extends EventEmitter3 {
  ...
  constructor(opts) {
    super();
    const { license, session: session2 = electronSession.defaultSession, modulePath, ...impl } = opts || {};
    checkVersion();
    ...
    this.modulePath = modulePath || path4.join(__dirname, "../..");
    this.api = {
  ...
  async prependPreload() {
    ...
    const preloadPath = path4.join(this.modulePath, "dist/preload.js");
    ...
    try {
      const stat = await fs4.stat(preloadPath);
