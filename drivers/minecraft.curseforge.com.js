...
request(temp.href, function(err, response, html) {
    ...
    request(temp.href, function(err, response, html) {
        ...
        if (current.md5 === temp.md5 && (current.file === temp.file || current.file === temp.file + ".disabled") && fs.existsSync((current["folder-override"] ? current["folder-override"] : obj.config.folder) + "/" + current.file)) { // Nothing to update.
