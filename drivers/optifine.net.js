...
request(current.url, function(err, response, html) {
    ...
    else temp.href = $("h2:contains(" + global.list.config.version + ")").nextAll("table.downloadTable").first().find("td.downloadLineMirror a").attr("href");
    ...
    if (!current.name) current.name = $("h2:contains(" + global.list.config.version + ")").next("h3").text().trim();
