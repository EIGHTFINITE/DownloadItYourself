	...
	const generateReadme = (t) => {
		...
		const renderDependencies = (modulesPath, moduleParents, moduleName, modulePack, type) => {
			...
			if(modulePack === 'package.json') {
				...
				while(pkg === null) {
					try {
						pkg = require(modulesPath + '/' + moduleParents.slice(0, -i++).join('/node_modules/') + '/node_modules/' + moduleName + '/' + modulePack)
					}
					catch(err) {
						console.log('Couldn\'t find ' + moduleName + ' at "'+ modulesPath + '/' + moduleParents.slice(0, -i++).join('/node_modules/') + '/node_modules/' + moduleName + '/' + modulePack + '"')
					}
				}
				// Source
				let depSource = ''
