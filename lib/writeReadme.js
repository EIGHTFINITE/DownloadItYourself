...
const writeReadme = (downloadlist) => {
	const generateReadme = (t) => {
		...
		const escapeUrl = (s,escapeSpace) => {
			s = s.replace(/\s+/g,' ').trim().replaceAll('%', '%25').replaceAll('[', '%5B').replaceAll(']', '%5D').replaceAll('@', '%40').replaceAll('!', '%21').replaceAll('$', '%24').replaceAll("'", '%27').replaceAll('(', '%28').replaceAll(')', '%29').replaceAll('*', '%2A').replaceAll('+', '%2B').replaceAll(',', '%2C').replaceAll(';', '%3B').replaceAll('"', '%22').replaceAll('<', '%3C').replaceAll('>', '%3E')
		...
		const textFromConfig = (textArray, type) => {
			...
			for (let i=0; i<textArray.length; i++) {
				...
				if((textArray[i].startsWith('https://') || textArray[i].startsWith('http://')) && textArray[i] !== textArray[i-1]) {
					...
					if(nextUrl.length === 1) {
						if(type === 'md') {
							string += '[' + textArray[i] + '](' + escapeUrl(nextUrl[0], true) + ')'
							...
							string += '<a href="' + escapeUrl(nextUrl[0]) + '">' + textArray[i] + '</a>'
							if(i>=(textArray.length-1)) {
								string += '</p>\n'
							}
						}
					...
					if(type === 'md') {
						string += '### ' + textArray[i].slice(4) + '\n\n'
					}
					else {
						string += '<h3>' + textArray[i].slice(4) + '</h3>\n'
					}
					forceNewLine = true
				}
				else if(textArray[i].startsWith('#### ')) {
					...
					if(type === 'md') {
						string += '#### ' + textArray[i].slice(5) + '\n\n'
					}
					else {
						string += '<h4>' + textArray[i].slice(5) + '</h4>\n'
					}
					forceNewLine = true
				}
				else if(textArray[i].startsWith('##### ')) {
					...
					if(type === 'md') {
						string += '##### ' + textArray[i].slice(6) + '\n\n'
					}
					else {
						string += '<h5>' + textArray[i].slice(6) + '</h5>\n'
					}
					forceNewLine = true
				}
				else if(textArray[i].startsWith('###### ')) {
					...
					if(type === 'md') {
						string += '###### ' + textArray[i].slice(7) + '\n\n'
					}
					else {
						string += '<h6>' + textArray[i].slice(7) + '</h6>\n'
					}
					forceNewLine = true
				}
				else {
