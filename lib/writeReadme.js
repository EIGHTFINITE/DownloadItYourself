				...
				if(textArray[i].startsWith('https://') && textArray[i] !== textArray[i-1]) {
					...
					if(nextUrl.length === 1) {
						if(type === 'md') {
							...
							string += (forceNewLine ? '<p>' : '') + '<a href="' + nextUrl[0] + '">' + textArray[i] + '</a>'
						}
						nextUrl = []
					}
					else {
						if(type === 'md') {
							...
							string += (forceNewLine ? '<p>' : '') + '<b>' + textArray[i] + '</b>'
							for(let j=0; j<nextUrl.length; j++) {
					...
					if(type === 'md') {
						string += textArray[i] + (nextIsUrl ? '' : '\n\n')
					}
					else {
						string += (prevWasUrl ? '' : '<p>') + textArray[i] + (nextIsUrl ? '' : '</p>\n')
					}
					prevWasUrl = false
					forceNewLine = false
				}
