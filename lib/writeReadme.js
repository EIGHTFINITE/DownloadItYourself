...
const writeReadme = (downloadlist) => {
	const generateReadme = (t) => {
		...
		// Node dependencies
		const renderDependencies = (packLocation, type) => {
			let string = ''
			let packDependencies = null
			if(packLocation.endsWith('/package.json')) {
				packDependencies = require(packLocation)
				const depIcon = ''
				const depHome = ''
				const depLocation = packDependencies.name
				const depAuthor = ''
				const depLicense = ''
				const depSource = ''
				...
				const depVersion = ''
				if(type === 'md') {
					string += '| [<img src="' + depIcon + '" width="31">](' + depHome + ')'
					string += ' | [' + depLocation + '](' + depHome + ')'
					string += ' | ' + depAuthor
					string += ' | ' + depLicense
					string += ' | [Open Source](' + depSource + ')'
					string += ' | &#x2714;&#xFE0F;'
					string += ' | ' + (depDescription !== '' ? (depRequiredBy !== '' ? depDescription + '<br>' + depRequiredBy : depDescription) : depRequiredBy)
					string += ' | ' + (depType === 'github' ? '`' + depResolved + '`<br>(based on `' + depVersion + '`)' : '`' + depVersion + '`') + ' |\n'
				}
				else {
					string += '<tr>\n'
					string += '<td align="center"><a href="' + depHome + '"><img src="' + depIcon + '" width="31"></a></td>\n'
					...
					string += '<td>' + (depDescription !== '' ? (depRequiredBy !== '' ? depDescription + '<br>' + depRequiredBy : depDescription) : depRequiredBy) +'</td>\n'
					string += '<td align="center">' + (depType === 'github' ? '<code>' + depResolved + '</code><br>(based on <code>' + depVersion + '</code>)' : '<code>' + depVersion + '</code>') + '</td>\n'
					string += '</tr>\n'
				}
				if(Array.isArray(packDependencies.dependencies)) {
					packDependencies = packDependencies.dependencies
				}
				else {
					packDependencies = []
				}
			}
			else if(!packLocation.endsWith('/.package-lock.json')) {
				throw Error('Unimplemented')
			}
			if(packDependencies === null) {
				packDependencies = Object.keys(require(packLocation).packages)
			}
			for(let i=0; i<packDependencies.length; i++) {
				const depIcon = ''
				const depHome = ''
				const depLocation = packDependencies[i]
				const depAuthor = ''
				const depLicense = ''
				const depSource = ''
				const depDescription = ''
