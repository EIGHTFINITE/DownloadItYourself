...
const writeReadme = (downloadlist) => {
	const generateReadme = (t) => {
		...
		// Node dependencies
		const renderDependencies = (modulesPath, moduleParent, moduleName, modulePack, type) => {
			let string = ''
			if(modulePack === 'package.json') {
				const pkg = require(modulesPath + '/' + (moduleParent === '' ? '' : moduleParent.replaceAll('/', 'node_modules/')+'/node_modules/') + moduleName + '/' + modulePack)
				// Source
				let depSource = ''
				if(pkg.repository) {
				...
				if(pkg.dependencies && (type !== 'md' || depLocation !== 'npm')) {
					...
					for(let i=0; i<depDependencies.length; i++) {
						...
						renderDependencies(modulesPath, (moduleParent === '' ? moduleName : moduleParent + '/' + moduleName), depDependencies[i], 'package.json', type)
					}
				}
				return string
			}
