	...
	const generateReadme = (t) => {
		const textFromConfig = (textArray, type) => {
			...
			for (let i=0; i<textArray.length; i++) {
				...
				if(i<(textArray.length-1) && textArray[i+1].startsWith('https://') && textArray[i].startsWith('https://') !== textArray[i+1].startsWith('https://')) {
					nextIsUrl = true
				}
				else {
					nextIsUrl = false
				}
				if(textArray[i].startsWith('https://') && textArray[i] !== textArray[i-1]) {
