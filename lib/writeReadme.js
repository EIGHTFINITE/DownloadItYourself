...
const writeReadme = (downloadlist) => {
	const generateReadme = (t) => {
		...
		const renderDependencies = (packLocation, type) => {
			...
			if(pkg === null) {
				pkg = Object.keys(require(packLocation).packages)
			}
			for(let i=0; i<pkg.length; i++) {
				const depIcon = ''
				const depHome = ''
				const depLocation = pkg[i].replaceAll('node_modules/','')
				const depAuthor = ''
				const depLicense = ''
				const depSource = ''
				const depDescription = ''
				...
				const depVersion = ''
				if(type === 'md') {
					string += '| [<img src="' + escapeUrl(depIcon) + '" width="31">](' + depHome + ')'
					string += ' | [' + depLocation + '](' + depHome + ')'
					string += ' | ' + depAuthor
					string += ' | ' + depLicense
					string += ' | [Open Source](' + depSource + ')'
