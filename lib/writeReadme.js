	...
	const generateReadme = (t) => {
		const traverseDependencies = (d, p, c) => {
			...
			for (const key of Object.keys(d)) {
				...
				else if(d[key].license === 'FreeBSD') {
					d[key].license = 'BSD-2-Clause-Views'
				}
				else if(pkg.name === 'bin-links' && d[key].license === 'Artistic-2.0') { // bin-links
					d[key].license = 'ISC'
				}
				...
				if(d[key].description === '' && pkg.name === 'corepack') { // corepack
