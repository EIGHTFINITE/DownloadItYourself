...
const writeReadme = (downloadlist) => {
	const generateReadme = (t) => {
		...
		// Node dependencies
		const renderDependencies = (modulesPath, moduleParents, moduleName, modulePack, type) => {
			let string = ''
			if(modulePack === 'package.json') {
				...
				try {
					...
					console.log('Couldn\'t find ' + moduleName + ' at "'+ modulesPath + '/' + (moduleParents.length > 0 ? moduleParents.join('/node_modules/') + '/node_modules/' : '') + moduleName + '/' + modulePack + '"')
				}
				let i = 0
				while(pkg === null) {
					if(i > moduleParents.length) {
						throw Error('Couldn\'t find ' + moduleName)
					}
					let moduleParentsSlice = moduleParents.slice(0, -i++)
					try {
						pkg = require(modulesPath + '/' + (moduleParentsSlice.length > 0 ? moduleParentsSlice.join('/node_modules/') + '/node_modules/' : '') + moduleName + '/' + modulePack)
					}
					catch(err) {
						console.log('Couldn\'t find ' + moduleName + ' at "'+ modulesPath + '/' + (moduleParentsSlice.length > 0 ? moduleParentsSlice.join('/node_modules/') + '/node_modules/' : '') + moduleName + '/' + modulePack + '"')
					}
				}
				// Source
				let depSource = ''
				...
				if(pkg.dependencies && (type !== 'md' || depLocation !== 'npm')) {
					...
					for(let j=0; j<depDependencies.length; j++) {
						if(depDependencies[j] === '') {
							continue
						}
						renderDependencies(modulesPath, moduleParents, depDependencies[j], 'package.json', type)
					}
				}
				return string
			}
