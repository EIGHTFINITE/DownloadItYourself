	...
	const generateReadme = (t) => {
		...
		const renderDependencies = (modulesPath, packLocation, type) => {
			...
			if(packLocation.endsWith('/package.json')) {
				...
				const depHome = ''
				const depLocation = 'node_modules/' + pkg.name
				const depLicense = ''
				...
				if(pkg.dependencies && (type !== 'md' || depLocation !== 'npm')) {
					...
					for(let i=0; i<depDependencies.length; i++) {
						...
						if(fs.existsSync(__dirname+'/'+modulesPath+'/'+depLocation+'/node_modules/'+depDependencies[i]+'/package.json')) {
							string += renderDependencies(modulesPath, modulesPath+'/'+depLocation+'/node_modules/'+depDependencies[i]+'/package.json', type)
						}
						else {
				...
				for(let i=0; i<depDependencies.length; i++) {
					...
					string += renderDependencies(modulesPath, modulesPath+'/'+depDependencies[i].slice(13)+'/package.json', type)
				}
				return string
			}
			else {
