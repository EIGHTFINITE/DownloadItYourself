...
const writeReadme = (downloadlist) => {
	const generateReadme = (t) => {
		...
		// Binaries
		const p7zipVersion = require('../downloadlist.json').config['p7zip-version']
		...
		// Node dependencies
		const renderDependencies = (modulesPath, moduleParents, moduleName, modulePack, hasSeen, type) => {
			let string = ''
			if(modulePack === 'package.json') {
				let pkg = null
				try {
					pkg = require(modulesPath + '/' + (moduleParents.length > 0 ? moduleParents.join('/node_modules/') + '/node_modules/' : '') + moduleName + '/' + modulePack)
				}
				catch(err) {
				...
				while(pkg === null) {
					if(moduleParents.length === 0) {
						throw Error('Couldn\'t find ' + moduleName)
					}
					moduleParents = moduleParents.slice(0, -1)
					try {
						pkg = require(modulesPath + '/' + (moduleParents.length > 0 ? moduleParents.join('/node_modules/') + '/node_modules/' : '') + moduleName + '/' + modulePack)
					}
					catch(err) {
				...
				if(hasSeen.includes(depVersion)) {
					return string
				}
				hasSeen.push(depVersion)
				// Location
				const depLocation = moduleName
				// Source
				let depSource = ''
				if(pkg.repository) {
				...
				depAuthor = depAuthor.replace(/\s+/g,' ').trim()
				let depAuthorMatch = depAuthor.match(/\(.*?\)$/)
				if(depAuthorMatch !== null && isInt(depAuthorMatch.index)) {
					depAuthor = depAuthor.slice(0, depAuthorMatch.index).trim()
				}
				depAuthorMatch = depAuthor.match(/<.*?>$/)
				if(depAuthorMatch !== null && isInt(depAuthorMatch.index)) {
					depAuthor = depAuthor.slice(0, depAuthorMatch.index).trim()
				}
				// Author corrections
				if(depAuthor === 'bdehamer@github.com') {
					depAuthor = 'Brian DeHamer'
				}
				else if(depLocation === 'corepack' && depAuthor === 'nodejs') {
					depAuthor = 'Corepack contributors'
				}
				// License
				let depLicense = 'All Rights Reserved'
				if(!pkg.license) {
					if(pkg.licenses) {
						if(pkg.licenses.length !== 1) {
							throw Error('Unimplemented')
						}
						if(isString(pkg.licenses[0].type)) {
							depLicense = pkg.licenses[0].type
						}
					}
				}
				else if(isString(pkg.license)) {
					depLicense = pkg.license
				}
				else if(isString(pkg.license.type)) {
					depLicense = pkg.license.type
				}
				// License corrections
				if(depLocation === 'npm' && depLicense === 'Artistic-2.0') {
					depLicense = 'npm license'
				}
				// Homepage
				const depHome = 'https://www.npmjs.com/package/' + pkg.name;
				// Description
				...
				if(pkg.dependencies && (type !== 'md' || depLocation !== 'npm')) {
					moduleParents.push(moduleName)
					const depDependencies = Object.keys(pkg.dependencies)
					for(let i=0; i<depDependencies.length; i++) {
						if(depDependencies[i] === '') {
							continue
						}
						string += renderDependencies(modulesPath, moduleParents, depDependencies[i], 'package.json', hasSeen, type)
					}
				}
				return string
			}
			else if(modulePack === 'package-lock.json') {
				const depDependencies = Object.keys(require(modulesPath + '/../' + modulePack).packages)
				for(let i=0; i<depDependencies.length; i++) {
					if(depDependencies[i] === '') {
						continue
					}
					string += renderDependencies(modulesPath, [], depDependencies[i].slice(13), 'package.json', hasSeen, type)
				}
				return string
			}
			else if(modulePack === '.package-lock.json') {
