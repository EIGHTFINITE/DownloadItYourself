...
const writeReadme = (downloadlist) => {
	const generateReadme = (t) => {
		...
		// Node dependencies
		const renderDependencies = (modulesPath, packLocation, type) => {
			let string = ''
			if(packLocation.endsWith('/package.json')) {
				const pkg = require(packLocation)
				// Source
				let depSource = ''
				if(pkg.repository) {
				...
				if(pkg.dependencies && (type !== 'md' || depLocation !== 'npm')) {
					...
					for(let i=0; i<depDependencies.length; i++) {
						...
						console.log(modulesPath+'node_modules/'+depLocation+'/node_modules/'+depDependencies[i]+'/package.json')
						if(fs.existsSync(modulesPath+'node_modules/'+depLocation+'/node_modules/'+depDependencies[i]+'/package.json')) {
							string += renderDependencies(modulesPath, modulesPath+'node_modules/'+depLocation+'/node_modules/'+depDependencies[i]+'/package.json', type)
						}
						else {
							string += renderDependencies(modulesPath, modulesPath+'node_modules/'+depDependencies[i]+'/package.json', type)
						}
					}
				}
				return string
			}
			else if(packLocation.endsWith('/package-lock.json') || packLocation.endsWith('/.package-lock.json')) {
				const depDependencies = Object.keys(require(packLocation).packages)
				for(let i=0; i<depDependencies.length; i++) {
					if(depDependencies[i] === '') {
						continue
					}
					string += renderDependencies(modulesPath, modulesPath+depDependencies[i]+'/package.json', type)
				}
				return string
			}
			else {
		...
		if(t === 'md') {
			...
			html += '| Icon | Name | Author | License | Source&nbsp;Code | Distribution | Description | Version |\n'
			html += '| :---: | --- | --- | --- | --- | :---: | --- | :---: |\n'
			html += renderDependencies('../bin/windows/x64/node/node-v'+nodeVersion+'-win-x64/', '../bin/windows/x64/node/node-v'+nodeVersion+'-win-x64/node_modules/corepack/package.json', t)
			html += renderDependencies('../bin/windows/x64/node/node-v'+nodeVersion+'-win-x64/', '../bin/windows/x64/node/node-v'+nodeVersion+'-win-x64/node_modules/npm/package.json', t)
