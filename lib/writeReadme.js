			...
			if(packLocation.endsWith('/package.json')) {
				...
				if(pkg.dependencies && (type !== 'md' || depLocation !== 'npm')) {
					...
					for(let i=0; i<depDependencies.length; i++) {
						...
						let newPackagePath = modulesPath+'node_modules/'+depLocation+'/node_modules/'+depDependencies[i]+'/package.json'
						try {
							require(newPackagePath)
						}
						catch(err) {
							if (err.code !== 'MODULE_NOT_FOUND') {
								throw err
							}
							newPackagePath = modulesPath+'node_modules/'+depDependencies[i]+'/package.json';
						}
						string += renderDependencies(modulesPath, newPackagePath, type)
					}
				}
				return string
			}
