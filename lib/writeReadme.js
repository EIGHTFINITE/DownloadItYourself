...
const writeReadme = (downloadlist) => {
	const generateReadme = (t) => {
		...
		const escapeUrl = (s,inHtml,escapeSpace) => {
			s = s.replace(/\s+/g,' ').trim().replaceAll('%', '%25').replaceAll('[', '%5B').replaceAll(']', '%5D').replaceAll('!', '%21').replaceAll('$', '%24').replaceAll("'", '%27').replaceAll('(', '%28').replaceAll(')', '%29').replaceAll('*', '%2A').replaceAll('+', '%2B').replaceAll(',', '%2C').replaceAll(';', '%3B')
				...
				if((textArray[i].startsWith('https://') || textArray[i].startsWith('http://')) && textArray[i] !== textArray[i-1]) {
					...
					if(nextUrl.length === 1) {
						if(type === 'md') {
							...
							string += '<a href="' + nextUrl[0] + '">' + textArray[i] + '</a>'
							if(i>=(textArray.length-1)) {
								string += '</p>\n'
							}
						}
