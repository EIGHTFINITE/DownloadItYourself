...
const writeReadme = (downloadlist) => {
	const generateReadme = (t) => {
		...
		// Node dependencies
		const renderDependencies = (basePath, packLocation, type) => {
			let string = ''
			if(packLocation.endsWith('/package.json')) {
				...
				if(pkg.dependencies && (type !== 'md' || depLocation !== 'npm')) {
					...
					for(let i=0; i<depDependencies.length; i++) {
						...
						if(fs.existsSync(__dirname+'/'+basePath+'node_modules/'+depLocation+'/node_modules/'+depDependencies[i]+'/package.json')) {
							string += renderDependencies(basePath, basePath+'node_modules/'+depLocation+'/node_modules/'+depDependencies[i]+'/package.json', type)
						}
						else {
							string += renderDependencies(basePath, basePath+'node_modules/'+depDependencies[i]+'/package.json', type)
						}
					}
				}
				return string
				...
				for(let i=0; i<depDependencies.length; i++) {
					...
					string += renderDependencies(basePath, basePath+depDependencies[i]+'/package.json', type)
				}
				return string
			}
			else {
