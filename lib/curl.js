	...
	const request = await new Promise((resolve) => {
		...
		fs.open(tempFileLocation, 'r', (err, fd) => {
			...
			fs.read(fd, buffer, 0, 4, 0, (err, num)  => {
				...
				const firstBytes = buffer.toString('utf8', 0, num).slice(0,4)
				if(firstBytes.length === 0) {
					resolve(Error('File is empty'))
					return
				}
				if(firstBytes === 'PK\x03\x04') { // zip
					resolve(null)
					return
				}
				else if(firstBytes.slice(0,2) === '[\n' || firstBytes.slice(0,2) === '{\n' || firstBytes.slice(0,2) === '{"') { // json
					...
					resolve(Error('Unrecognized start of file: "' + [...firstBytes].map(u => u.charCodeAt(0).toString(16).toUpperCase().padStart(2, '0')).join(' ') + '"'))
					return
				}
			})
		})
